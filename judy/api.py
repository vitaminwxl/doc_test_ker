...
def disk_status(host_name, memory_total, cpu_total, images, disk_size, disk_type_name, host_ip, container_ip,container_gw):
    ...
    args_centos7 = {
        ...
        "stdin_open": True,
        #"cmd": "none",
        #"command": "-n -c /etc/supervisord.conf"
    ...
    try:
            ...
            try:
                ...
                ssh_c = pexpect.spawn('ssh -p21987  zeusadmin@%s "%s"' % (host_ip, 'sudo /bin/pipework em2 -i eth2 %s %s@%s' % (host_name, container_ip, container_gw)))
                i = ssh_c.expect(['password:', 'continue connecting (yes/no)?'])
                #print i
                #print type(i)
                if i == 0:
...
#print disk_status('python-docker191', int('4000000000'),'0-3','centos7','1GB','sdc','10.69.111.9','10.69.113.192/24','10.69.113.254')
# print disk_status()
